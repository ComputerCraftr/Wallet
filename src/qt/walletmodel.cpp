...
bool WalletModel::validateAddress(const QString& address)
{
    ...
    return addressParsed.IsValid() /*|| (address.toStdString().length() > 75 && IsStealthAddress(address.toStdString()))*/;
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered beetlecoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BEETLECOIN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal beetlecoin:URI (beetlecoin:XyZ...?message=example)
            {
...
static void NotifyzBEETReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBEETReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBEETReset.connect(boost::bind(NotifyzBEETReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBEETReset.disconnect(boost::bind(NotifyzBEETReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbeetTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
