...
public:
    ...
    bool WriteZBEETSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBEETSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBEETSeed_deprecated(uint256& seed);
    bool EraseZBEETSeed();
    bool EraseZBEETSeed_deprecated();

    bool WriteZBEETCount(const uint32_t& nCount);
    bool ReadZBEETCount(uint32_t& nCount);
